 ...
 * //////////////////////////////////////////////////////////////////////////////////////
 */
class Emipro_SellerpriceRules_Model_Rule extends Mage_CatalogRule_Model_Rule
{
   /**
    ...
    {
		//$ruleCollection=Mage::getModel('catalogrule/rule')->load($this->getId());$ruleId
		$vendorId= Mage::helper("marketplace")->getProuctVendorid($this->getSellerAdminId());//$ruleCollection->getSellerAdminId()
		...
		if (is_null($this->_productIds)) {
            ...
            if ($this->getWebsiteIds()) {
				...
				if($vendorId=="")
				{
					$modeldata = Mage::getModel('marketplace/list')->getCollection();
					$vendorids = $modeldata->getColumnValues('product_vendor_id');
					$productsCollection=Mage::getModel('catalog/product')->getCollection();
					$productIds=$productsCollection->addFieldToFilter("vendor_id",array('in' => array($vendorids)))->getAllIds();
					$productCollection->addAttributeToFilter("entity_id",array("nin"=>array($productIds)));
				}
				else
				{
					$productCollection->addAttributeToFilter("vendor_id",$vendorId);
				}
    ...
    public function applyAllRulesToProduct($product)
    {
		...
		/***** Rule filter *****/      
        $rules = Mage::getModel('catalogrule/rule')->getCollection()->addFieldToFilter("seller_admin_id",$SellerId)
