...
class Mana_Core_Model_Observer {
	...
	public function postProcessBlocks($observer) {
		...
		//Checking the page has manufacturing_types id to set meta title and meta description
		if(Mage::app()->getRequest()->getParam('manufacturing_types')){
			$int_attr_id = 183;
			$int_attr_value = Mage::app()->getRequest()->getParam('manufacturing_types');
			$attributes = Mage::getModel('catalog/resource_eav_attribute')->load($int_attr_id);
			$titleLabel = 'Machine Type: '.$attributes->getSource()->getOptionText($int_attr_value).' Supplier, Wholesaler & Traders India';
			$titleLabelDesc = $attributes->getSource()->getOptionText($int_attr_value).' Supplier, Wholesaler & Traders India';
			$descriptionLabel = 'Browse Metacrust E-shop for all of your '.$titleLabelDesc;
			if(!Mage::app()->getRequest()->getParam('m-show-more-popup')){
				$head->setMetaTitle($titleLabel);
				$head->setMetaDescription($descriptionLabel);
			}
		}
		//Checking the page has category_type id to set meta title and meta description
		if(Mage::app()->getRequest()->getParam('category_type')){
			$int_attr_id = 182;
			$int_attr_value = Mage::app()->getRequest()->getParam('category_type');
			$attributes = Mage::getModel('catalog/resource_eav_attribute')->load($int_attr_id);
			$titleLabel = 'Industry Type: '.$attributes->getSource()->getOptionText($int_attr_value).' Equipment Supplier India';
			$titleLabelDesc = $attributes->getSource()->getOptionText($int_attr_value).' Equipment Supplier India';
			$descriptionLabel = 'Browse the Metacrust E-shop for your entire '.$titleLabelDesc;
			if(!Mage::app()->getRequest()->getParam('m-show-more-popup')){
				$head->setMetaTitle($titleLabel);
				$head->setMetaDescription($descriptionLabel);
			}
		}
		//Checking the page has brands id to set meta title and meta description
		if(Mage::app()->getRequest()->getParam('brands')){
			$int_attr_id = 181;
			$int_attr_value = Mage::app()->getRequest()->getParam('brands');
			$attributes = Mage::getModel('catalog/resource_eav_attribute')->load($int_attr_id);
			$titleLabel = 'Brands: '.$attributes->getSource()->getOptionText($int_attr_value).' Products & Equipment Suppliers in India';
			$titleLabelDesc = $attributes->getSource()->getOptionText($int_attr_value).' Products & Equipment Suppliers in India';
			$descriptionLabel = 'Browse the Metacrust E-shop for your entire '.$titleLabelDesc;
			if(!Mage::app()->getRequest()->getParam('m-show-more-popup')){
				$head->setMetaTitle($titleLabel);
				$head->setMetaDescription($descriptionLabel);
