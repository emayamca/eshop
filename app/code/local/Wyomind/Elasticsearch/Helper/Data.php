 ...
 */
class Wyomind_Elasticsearch_Helper_Data extends Mage_Core_Helper_Abstract
{
     ...
     * @var array
     */
    protected $_includedAttributes = array('visibility', 'image', 'tax_class_id', 'product_weight');
     ...
     *
     * @param mixed $store
     * @return Wyomind_Elasticsearch_Model_Resource_Client
     */
...
//        unset($analyzer);
        ...
        Mage::dispatchEvent('wyomind_elasticsearch_index_settings', array(
            'client' => $this,
            'store' => $store,
    ...
    public function getCategoryCollection($queryText, $store = null)
    {
        ...
        /** @var Wyomind_Elasticsearch_Model_Resource_Engine $engine */
     ...
     */
    public function getPageCollection($queryText,
            $store = null)
    {
        $store = Mage::app()->getStore($store);
        $pageIds = array();
     ...
     * @return array
     */
    public function getSearchableAttributesConfig($entity,
            $store = null)
    {
     ...
     * @return array
     */
    public function getSearchConfigData($field,
            $store = null)
    {
            ...
            $store = null)
    {
        ...
        if ($this->isActiveEngine($store)) {
            ...
            $select = $adapter->select()
                    ...
                    ->joinInner(
                            array('e' => $resource->getTable('catalog/product')), 'e.entity_id = s.product_id', array()
                    )
    ...
    public function getStoreTypes($store = null)
    {
        ...
        Mage::dispatchEvent('wyomind_elasticsearch_store_types', array(
            'store' => $store,
            'types' => $types,
        ));
    ...
    public function getSupportedLanguages()
    {
        ...
        Mage::dispatchEvent('wyomind_elasticsearch_supported_languages', array(
            'languages' => $languages,
        ));
    ...
    public function isActiveEngine($store = null)
    {
        ...
        if ($engine && Mage::getConfig()->getResourceModelClassName($engine)) {
            ...
            return $model && $model instanceof Wyomind_Elasticsearch_Model_Resource_Engine && $model->test($store);
        }
    ...
    {
        return ($attribute->getIsSearchable() || $attribute->getIsVisibleInAdvancedSearch() || in_array($attribute->getAttributeCode(), $this->_includedAttributes));
    }
    ...
    public function isAttributeUsingOptions($attribute)
    {
        ...
        return $attribute->usesSource() && ($backend == 'int' && $model instanceof Mage_Eav_Model_Entity_Attribute_Source_Table) || (($backend == 'varchar' ||$backend == 'text') && $attribute->getFrontendInput() == 'multiselect');
    }
    ...
    {
        return Mage::helper('catalogsearch')->getEngine() instanceof Wyomind_Elasticsearch_Model_Resource_Engine;
    }
     ...
     * @return bool
     */
    public function isSearchEnabled($entity,
            $store = null)
    {
     ...
     * @return bool
     */
    public function isIndexationEnabled($entity,
            $store = null)
    {
     ...
     * @return mixed
     */
    protected function _formatValue($attribute,
            $value,
            $store = null)
    {
            ...
            if ($value == 'no_selection') {
                ...
                try {
                    ...
                    $value = Mage::getDesign()->setStore($store)->getSkinUrl(
                            'images/catalog/product/placeholder/image.jpg', array('_area' => 'frontend')
                    );
                }
            }
        }
